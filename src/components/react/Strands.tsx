...
export default function Strands({
  ...
  useEffect(() => {
    ...
    const renderFrame = (t: number) => {
      const c = propsRef.current as Record<string, number | string[] | boolean>;
      program.uniforms.uTime.value = t * 0.001;
      program.uniforms.uColors.value = buildPalette(c.colors as string[]);
      program.uniforms.uColorCount.value = Math.min((c.colors as string[]).length, MAX_COLORS);
      program.uniforms.uStrandCount.value = Math.min(Math.max(Math.round(c.count as number), 1), MAX_STRANDS);
      ...
      if (c.glass) {
        ...
        glassProgram.uniforms.uRadius.value = 0.46 * (c.glassSize as number);
